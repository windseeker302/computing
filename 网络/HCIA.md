...
- **做什么：** 路由器开机后，像按喇叭一样，不断向外喊：“你好，我是R1！”
- **结果：** 和它直接相连的路由器听到后，就会互相认识，成为“邻居”。

2. 同步地图目录（交换DBD包）

- **做什么：** 成为邻居后，互相交换一份“地图目录”，告诉对方：“我的地图里有这些路段信息。”
- **结果：** 知道对方有哪些地图信息，自己又缺哪些。

3. 共享地图信息（泛洪LSA）

- **做什么：** 路由器把自己知道的路况信息（比如“我这条路有多长、通向谁”）打包成一个“路况通告”(LSA)，然后“病毒式”地传给区域里所有的路由器。
- **结果：** 经过一轮信息轰炸，**区域内每台路由器的“路况数据库”(LSDB)都变得一模一样**。这就等于每台路由器都有了一份完整的、全网的实时交通地图。

4. 计算最佳路径（运行SPF算法）

- **做什么：** 在这张完整的地图上，每台路由器以自己为出发点，使用 **Dijkstra 算法**（就像导航软件计算最短路径）计算出到所有目的地的最佳路线。
- **结果：** 将算出来的最佳路径（比如“去A地，走3号出口”）写入自己的**路由表**，用于指导数据包的转发。
